...
export default function ShowCategory() {
  const { id } = useParams();
  const {data:category, isLoading, isError} = useGetCategory(Number(id));

  const getProductsByCategoryId = useDashboardStore(
    (state) => state.getProductsByCategoryId,
  );
  ...
  return (
    <div className="m-8">
      ...
      {/* products section  */}
      <div>
        <h2 className="font-semibold md:text-xl mb-4">
